...
const CoursePlayer = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [courseRes, lessonsRes, enrollmentsRes] =
          await Promise.all([
  ...
  // Check whether a lesson has a quiz
  const checkLessonQuiz = async (lessonId) => {
    try {
      ...
      setLessonQuiz(response.data);
    } catch (error) {
      // 404 simply means this lesson has no quiz
      if (error.response?.status === 404) {
        setLessonQuiz(null);
      } else {
        console.error('Failed to check lesson quiz:', error);
        setLessonQuiz(null);
      }
    } finally {
  ...
  const handleLessonSelect = async (lesson) => {
    ...
    // Only check for quiz if the lesson is completed
  ...
  const handleLessonComplete = async (lessonId, completed) => {
    try {
      ...
      if (completed) {
        ...
        await checkLessonQuiz(lessonId);
      } else {
        setLessonQuiz(null);
      }
    } catch (error) {
              ...
              <div>
                ...
                <div className="flex flex-col items-end gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <label
                      htmlFor={`complete-${currentLesson.id}`}
